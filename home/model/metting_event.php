<?
class Metting_event extends Base{
   var $table = 'metting_event';
   var $id_field = 'met_id';
   var $name_field = 'met_title';
   var $page;
   var $page_size = 2;
   var $result = array();
   public function __construct($alias){
        ...
        if($this->result) {   
            $this->name = $this->result['met_title'];
            $this->description = $this->result['met_summary'];
            $this->url = 'tin-tuc/'.$this->result['met_alias'].'.html';
        }
        
        parent::__construct($this->result['met_id']);
    }
   public function list_news() {  
      ...
      return db_array('SELECT *
                       FROM metting_event
                       WHERE met_active = 1 AND met_cat_id IN ('.$list_cat_child_id.')
                       LIMIT '. $page_start . ', '. $this->page_size .'');*/
      return db_array('SELECT *
                          FROM metting_event
                          WHERE met_active = 1 ORDER BY met_id DESC
                          LIMIT '. $page_start . ', '. $this->page_size .'');
   }
   public function total_news(){
      $arr = db_array('SELECT met_id
                     FROM metting_event
                     WHERE met_active = 1
                     ');
      return count($arr);
   ...
   public function news_hot() { 
      return db_array('SELECT *
                          FROM metting_event
                          WHERE met_active = 1
                          AND met_hot = 1 ORDER BY met_id DESC LIMIT 4');
   }
   public function cat_news() {  
      return db_array('SELECT *
                          FROM categories
                          WHERE cat_active = 1
                          AND cat_type="metting_event"
                          AND cat_parent_id = 0
                          ORDER BY cat_id ASC');
   }
   public function cat_count_news($cat_id) {  
      ...
      $list_news_cat = db_array('SELECT met_id
                             FROM metting_event
                             WHERE met_active = 1 AND met_cat_id IN ('.$list_cat_child_id.')');
   ...
   public function news_connection($met_date) {  
      $list_news_cat = db_array('SELECT *
                             FROM metting_event
                             WHERE met_active = 1 AND met_date < '.$met_date.' 
                             LIMIT 5');
