...
if($iQuick == 'update'){
	...
	if($total_record > 0){
		for($i = 0; $i < $total_record; $i++){
         ...
         $pag_title = getValue("pag_title" . $record_id[$i],'str','POST','');
         $pag_alias = getValue("pag_alias" . $record_id[$i],'str','POST','');
         $pag_alias = get_alias($pag_alias,$pag_title,$bg_table,$id_field,$alias_field,$record_id[$i]);
         $myform->add('pag_alias','pag_alias',0,1,'');
         
			//Insert to database
         $myform->add('pag_title', "pag_title" . $record_id[$i], 0, 0, '', 0, '', 0, '');
			//Add table
