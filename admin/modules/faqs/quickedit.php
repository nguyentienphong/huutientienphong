...
if($iQuick == 'update'){
	...
	if($total_record > 0){
		for($i = 0; $i < $total_record; $i++){
         ...
         $myform->add('faq_cat_id', "faq_cat_id" . $record_id[$i], 1, 0, '', 0, '', 0, '');
         $myform->add('faq_title', "faq_title" . $record_id[$i], 0, 0, '', 0, '', 0, '');
			//Add table
			$myform->addTable($bg_table);
			
			$upload_pic = new upload("faq_image" . $record_id[$i], $bg_filepath, $bg_extension, $limit_size);
			$file_name	=	$upload_pic->file_name;
			//echo	$file_name;
			if($file_name != ""){
				$myform->add("faq_image","file_name", 0, 1, "", 0, "", 0, "");
				foreach($arr_resize as $type => $arr){
					resize_image($bg_filepath, $file_name, $arr["width"], $arr["height"], $arr["quality"], $type);
				}
				delete_file($bg_table,$id_field,$record_id[$i],"faq_image",$bg_filepath);
			}
