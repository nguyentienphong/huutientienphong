...
if($iQuick == 'update'){
	...
	if($total_record > 0){
		for($i = 0; $i < $total_record; $i++){
         ...
         $pos_title = getValue("pos_title" . $record_id[$i],'str','POST','');
         $pos_alias = getValue("pos_alias" . $record_id[$i],'str','POST','');
         $pos_alias = get_alias($pos_alias,$pos_title,$bg_table,$id_field,$alias_field,$record_id[$i]);
         $myform->add('pos_alias','pos_alias',0,1,'');
         
			//Insert to database
         $myform->add('pos_title', "pos_title" . $record_id[$i], 0, 0, '', 0, '', 0, '');
			$myform->add('pos_cat_id', "pos_cat_id" . $record_id[$i], 1, 0, '', 0, '', 0, '');
			//Add table
