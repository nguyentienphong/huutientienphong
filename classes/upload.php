...
class upload{
	...
	$min_width		   :    kich co chieu rong nho nhat
   $min_height		   :    kick co chieu cao nho nhat
   $resize100        :    resize 100% kick thước để xóa bỏ các ký tự nghi vấn hack {true:có resize ,false : không resize}
	*/
	var $common_error		= "";
	var $warning_error	= "";
	var $file_name			= "";
	...
	function upload($upload_name, $upload_path, $extension_list, $limit_size, $arr_multi= null ,$prefix_img = "" ,$resize100 = "false"){
	  if($arr_multi) $_FILES[$upload_name] = $arr_multi;
	  if(is_array($_FILES[$upload_name]['tmp_name'])){
         $filesCount =  count($_FILES[$upload_name]['tmp_name']);
         for($i=0;$i<$filesCount;$i++){
            ...
            //check chieu cao/rong
           
            //Generate new filename
            $new_filename					= $prefix_img.$this->generate_name($_FILES[$upload_name]['name'][$i]);
            ...
            // check nếu là ảnh thì resize 100% kich thước(để xóa bỏ các ký tự nghi vấn)
            if($resize100 == 'true'){
               if(array_search($this->getExtension($new_filename), $this->image_extension)){
                  $this->resize_image($upload_path,$new_filename,10000,10000,"",$upload_path);
               }
            }
         }
	  }else{
         ...
         //Generate new filename
         $new_filename					= $prefix_img.$this->generate_name($_FILES[$upload_name]['name']);
         $this->file_name				= $new_filename;
         //move upload file
	...
	*/
	...
	/**
    * @path : path cu~ cua anh
    * @filename : ten file anh
    * @newwidth : max width moi
    * @newheight  : max height moi
    * @maintainRatio : giu nguyen ty le cu hay khong
    * @prefix : prefix ten anh moi
    * @newpath : path moi
    * */
